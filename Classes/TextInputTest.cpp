...
void TextInputTest::onEnter()
{
    ...
    addChild(label, 9999);
    label->setPosition( Point(240, 240) );
    ...
    if( ! strSubtitle.empty() )
    {
        ...
        addChild(l, 9999);
        l->setPosition( Point(240, 200) );
    }

    // add menu
	// CC_CALLBACK_1 == std::bind( function_ptr, instance, std::placeholders::_1, ...)
    MenuItemImage *item1 = MenuItemImage::create("CloseNormal.png", "CloseSelected.png", CC_CALLBACK_1(TextInputTest::backCallback, this) );
    MenuItemImage *item2 = MenuItemImage::create("CloseNormal.png", "CloseSelected.png", CC_CALLBACK_1(TextInputTest::restartCallback, this) );
    MenuItemImage *item3 = MenuItemImage::create("CloseNormal.png", "CloseSelected.png", CC_CALLBACK_1(TextInputTest::nextCallback, this) );

    Menu *menu = Menu::create(item1, item2, item3, NULL);

    menu->setPosition(Point::ZERO);
    item1->setPosition(Point(100, 50));
    item2->setPosition(Point(200, 50));
    item3->setPosition(Point(300, 50));
...
cocos2d::Node * KeyboardNotificationLayer::_trackNode = NULL;
...
void TextFieldTTFDefaultTest::onClickTrackNode(bool bClicked)
{
    ...
    else
    {
        ...
        pTextField->detachWithIME();
				CCLog("GET TEXT Default: %s", pTextField->getString());
    }
}
...
void TextFieldTTFActionTest::onClickTrackNode(bool bClicked)
{
    ...
    else
    {
        ...
        pTextField->detachWithIME();
				CCLog("GET TEXT ACTION: %s", pTextField->getString());
    }
}
...
bool TextFieldTTFActionTest::onTextFieldDetachWithIME(TextFieldTTF * sender)
{
    if (_action)
    {
		...
		CCLog("GET TEXT: %s", _textField->getString());
    return false;
}
...
Layer* TextInputTestScene::runThisTest()
{
...
const char* TextInputTestScene::getText()
{
	TextFieldTTF * pTextField = (TextFieldTTF*)(KeyboardNotificationLayer::_trackNode);
